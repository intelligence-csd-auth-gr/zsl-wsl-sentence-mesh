...
def my_predictor_save(biobert, x_test, label_emb, th = 0.77):
	...
	for i in range(0,len(x_test)):
		...
		for sent in tokens.sents:
			...
			if(len(sent.string.strip())>10):
				...
				if(dist >= Max):
					 Max = dist
				df['label1_instance:' + str(i+1)].append(dist)
	...
	#for _ in reject:
	#	del df['label1_instance' + str(_+1)]
	...
	t = np.delete( np.array(x_test) , reject)
	y = np.delete( np.array(y_pred) , reject)
	
	t_new = []
	for i in list(t):
		t_new.append(str(i))

	return list(y), df, t_new, total_calls, total_sent
...
for label in labels:
	...
	for sc in scenario:
		...
		threshold = [0.77] #np.arange(0.65, 0.91, 0.01)
		...
		if mode == 'save_embs':
			...
			predictions_train, df_train, x_train_new, total_calls_train, total_sent_train = my_predictor_save(biobert, x_train, label_emb, th)
			predictions_test,  df_test,  x_test_new,  total_calls_test , total_sent_test  = my_predictor_save(biobert, x_test,  label_emb, th)

			end1 = time.time()

			#y_test_edited = y_test_edited1
			with open('BERT_per_sentence_' + label + '_' + sc + '_' + mode + '.pickle', 'wb') as f:
					pickle.dump([df_train, df_test, predictions_train, predictions_test, x_train_new, x_test_new, [total_calls_train, total_sent_train] , (end1-start1)], f)
		...
		if mode == 'mode1':
			...
			y_train_occ = occurence(x_train_new, y_train)
			x_train_tf, x_test_tf = tfidf(x_train_new, y_train, x_test_new)
			...
			#lea = Grad()
			#lea = ADA()
			learner = 'SVC' #Grad' #SVC' # LR' #'linearSVC'
			...
			label_emb= np.array(torch.stack(biobert.word_vector(label)))[0]
			predictions , density = state_of_the_art_predictor(x_test,0.77,3,label_emb)
		...
		for th in threshold:
			...
			if mode == 'mode1':
				...
				predictions, ddf1, x_train_new, total_calls_train, total_sent_train = my_predictor_save(biobert, x_test, label_emb, th)
				...
				y_test_edited = y_test_edited2
					...
					if mode == 'mode3':
								...
								y_train_bert, reject_th = augment_y_with_embeddings_max(biobert, label_emb, x_train_bert1, y_train, 2, threshold = np.arange(0.65, 0.91, 0.01))
								#print(y_train_bert)
								#break
								...
								#print(x_train_bert1)
								y_train_mode4, reject_th = augment_y_with_embeddings_max(biobert, label_emb, x_train_bert1, y_train, 2, threshold = np.arange(0.65, 0.91, 0.01))
					...
					lea = ADA()
					learner = 'Ada_range' #c_10_prob' #1nn' #RF' #Grad' #'SVC_tuned' # LR' 'linearSVC'
