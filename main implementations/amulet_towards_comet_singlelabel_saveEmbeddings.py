...
def my_predictor_save_old(biobert, x_test, label_emb, th = 0.73):
...
def my_predictor_save(biobert, x, label_emb1, label_emb2, label_emb3):
	...
	for i in range(0,len(x)):
		...
		df1['label1_instance:' + str(i)] = []
		df2['label1_instance:' + str(i)] = []
		df3['label1_instance:' + str(i)] = []
		...
		for sent in tokens.sents:
			...
			if(len(sent.string.strip())>10):
				...
				calls += 1
				dist1 = 1-distance.cosine(label_emb1,np.array(biobert.sentence_vector(sent.string.strip())))
				dist2 = 1-distance.cosine(label_emb2,np.array(biobert.sentence_vector(sent.string.strip())))
				dist3 = 1-distance.cosine(label_emb3,np.array(biobert.sentence_vector(sent.string.strip())))

				
				#if(dist1 >= Max1):
				#	 Max1 = dist1
				df1['label1_instance:' + str(i)].append(dist1)
				#if(dist2 >= Max2):
				#	 Max2 = dist2
				df2['label1_instance:' + str(i)].append(dist2)
				#if(dist3 >= Max3):
				#	 Max3 = dist3
				df3['label1_instance:' + str(i)].append(dist3)
	...
	for i in df1.keys():
		if df1[i] == []:
			...
			reject1.append(i[ i.find(':') + 1 : ])
			del df1[i]
	for i in df2.keys():
		if df2[i] == []:
			print('Empty sentence: ', i)
			reject2.append(i[ i.find(':') + 1 : ])
			del df2[i]
	for i in df3.keys():
		if df3[i] == []:
			print('Empty sentence: ', i)
			reject3.append(i[ i.find(':') + 1 : ])
			del df3[i]
	
	reject1 = list(set(reject1))
	reject2 = list(set(reject2))
	reject3 = list(set(reject3))
	print(reject1, reject2, reject3)

	t = np.delete( np.array(x) , reject1)
	
	t_new = []
	for i in list(t):
		t_new.append(str(i))

	return [df1, df2, df3], t_new, total_calls, total_sent, x, [reject1, reject2, reject3]
...
def state_of_the_art_predictor(x_test,cmax,label_embedding1, label_embedding2, label_embedding3):
	...
	for i in range(0,len(x_test)):
		...
		density1[i], density2[i], density3[i] = {}, {}, {}
		abstract = x_test[i]
		final_max = 0
		...
		for j in range(1,(cmax+1)):
			...
			density1[i][j] , density2[i][j] , density3[i][j] = [], [], []

			for sentence in splitTextToNumber(abstract,j):

				distj1 = 1-distance.cosine(label_embedding1,np.array(biobert.sentence_vector(sentence.strip())))
				distj2 = 1-distance.cosine(label_embedding2,np.array(biobert.sentence_vector(sentence.strip())))
				distj3 = 1-distance.cosine(label_embedding3,np.array(biobert.sentence_vector(sentence.strip())))

				density1[i][j].append([distj1])
				density2[i][j].append([distj2])
				density3[i][j].append([distj3])
...
for label in labels:
	...
	for sc in scenario:
		...
		if sc == 'train_ratio_1_1_test_ratio_pure':
			...
			x_test,y_test   = prepare_X_Y("negative_test.txt")
		...
		if mode == 'save_embs_negative':
			ls = ["Biomineralization" , "Chlorophyceae" , "Cytoglobin"]
			print('reduce time mode')
			start1 = time.time()

			label_emb1 = np.array(torch.stack(biobert.word_vector(ls[0])))[0]
			label_emb2 = np.array(torch.stack(biobert.word_vector(ls[1])))[0]
			label_emb3 = np.array(torch.stack(biobert.word_vector(ls[2])))[0]


			#df_train, x_train_new, total_calls_train, total_sent_train, x_train_old, reject_train = my_predictor_save(biobert, x_train, label_emb)
			df_tests,  x_test_new , total_calls_test , total_sent_test , x_test_old , reject_tests  = my_predictor_save(biobert, x_test,  label_emb1, label_emb2, label_emb3)

			#predictions_train, df_train, reject_tr, x_train_new = my_predictor_save(biobert, x_train, label_emb)
			#predictions_test, df_test, reject_ts, x_test_new = my_predictor_save(biobert, x_test,  label_emb)
			end1 = time.time()

			#y_test_edited = y_test_edited1
			with open('BERT_per_sentence_' + label + '_' + sc + '_' + mode + '.pickle', 'wb') as f:
					pickle.dump([df_tests, y_test, x_test_new, x_test_old, reject_tests, [total_calls_test, total_sent_test] , (end1-start1)], f)
					#pickle.dump([df_train, df_test, predictions_train, predictions_test, x_train_new, x_test_new, [total_calls_train, total_sent_train, total_calls_test, total_sent_test] , (end1-start1)], f)
					#pickle.dump([df_train, df_test, predictions_train, predictions_test, x_train_new, x_test_new, reject_tr, reject_ts] , f)
			f.close()
			continue
		...
		if mode == 'mode1':
			...
			with open('BERT_per_sentence_' + label +  '_' + sc + '_save_embsnew.pickle', 'rb') as f:
				...
				#x_train_bert_profile, x_test_bert_profile, x_train_bert1_077, x_test_bert1_077, x_train_new, x_test_new, [total_calls_train, total_sent_train], time_preprocess2 = pickle.load(f)
			...
			with open('BERTemb_' + sc +  '_' + label + '.pickle', 'rb') as f:
			...
			with open("BERTemb_" + sc + '_' + label + 'new.pickle' , 'wb') as f:
				pickle.dump([x_train_bert, x_test_bert, time_preprocess1, x_train_new1, x_test, y_train_new1, y_test] ,f,  protocol=pickle.HIGHEST_PROTOCOL)
			...
			with open('BERT_per_sentence_negative_negative_save_embs_negative.pickle', 'rb') as f:
						df_tests, y_test_new, x_test_new, x_test_old, reject_tests, [total_calls_test, total_sent_test], time_preprocess = pickle.load(f)
			f.close()
			#x_train = x_train_new
			#x_test = x_test_new
			ls = ["Biomineralization" , "Chlorophyceae" , "Cytoglobin"]

			#x_test_new = x_test_new[0:2]
			label_emb1 = np.array(torch.stack(biobert.word_vector(ls[0])))[0]
			label_emb2 = np.array(torch.stack(biobert.word_vector(ls[1])))[0]
			label_emb3 = np.array(torch.stack(biobert.word_vector(ls[2])))[0]

			density1, density2, density3 = state_of_the_art_predictor(x_test_new,3,label_emb1, label_emb2, label_emb3)
			...
			with open('density_' + label + '_' + sc + '_' + mode + '.pickle', 'wb') as f:
					pickle.dump([density1, density2, density3, y_test_edited], f)
			f.close()
